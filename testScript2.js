...
socket.on('__deviceState', function(device){// it executes by commands from front-end
...
setInterval(function Test(){
        var test = Math.floor(Math.random() * 3) + 1; // test is anything between 1 to 3
        
        
        if(test === 1){ //execute test case 1
            test1();
        }else if(test === 2){
            test2();
        }else if(test === 3){
...
}, 1000*60*6);  //run each test after 6mins


function test1(){
    console.log("starting test 1");
    async.series({
        NormalCall: function(cb){
            setTimeout(function(){
            state.value = 1;
            executeState();
            cb(null,1);
        ...
        Presence: function(cb){
            setTimeout(function(){
                state.value = 0;
                executeState();
                cb(null,0);
            }, 1000 * 60 * 1); //answers after 1minits
